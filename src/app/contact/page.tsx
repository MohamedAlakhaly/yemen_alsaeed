...
import { db } from "@/app/firebaseConfig.js" // <-- ملف firebase.js اللي سويناه
      ...
      <main className="min-h-screen bg-gradient-to-b from-background to-background/90 relative overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="firstName" value={form.firstName} onChange={handleChange} placeholder="Your first name" />
                    </div>
                    <div>
                      <label htmlFor="lastName" className="block text-sm font-medium mb-2">Last Name</label>
                      <Input id="lastName" value={form.lastName} onChange={handleChange} placeholder="Your last name" />
                    </div>
                  </div>

                  <div>
                    <label htmlFor="email" className="block text-sm font-medium mb-2">Email</label>
                    <Input id="email" type="email" value={form.email} onChange={handleChange} placeholder="your.email@example.com" />
                  </div>

                  <div>
                    <label htmlFor="phone" className="block text-sm font-medium mb-2">Phone Number</label>
                    <Input id="phone" type="tel" value={form.phone} onChange={handleChange} placeholder="+32 *** ** ** **" />
                  </div>

                  <div>
                    <label htmlFor="message" className="block text-sm font-medium mb-2">Message</label>
                    <Textarea id="message" value={form.message} onChange={handleChange} placeholder="Tell us how we can help you..." className="min-h-[120px]" />
                  </div>
        ...
        <section className="container mx-auto py-16 px-6 sm:px-8 md:px-12 lg:px-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: MapPin, title: "Visit Us", description: "Bd Jamar 17, 1060 Saint-Gilles", detail: "Open daily for dine-in" },
              { icon: Phone, title: "Call Us", description: "+32 465 41 18 84", detail: "Available 24/7" },
              { icon: Mail, title: "Email Us", description: "info@yemenalsaid.com", detail: "We reply within 2 hours" },
              { icon: Clock, title: "Opening Hours", description: "Every day: 12AM - 12PM", detail: "Kitchen closes at 11:30PM" }
            ].map((contact, i) =>
              <Card key={i} className="bg-background/50 backdrop-blur-sm border-border/50 hover:border-primary/50 transition-all duration-300 group hover:shadow-lg">
                ...
                <CardContent>
                  <p className="font-medium text-foreground mb-1">{contact.description}</p>
        ...
        <section className="container mx-auto py-16 px-6 sm:px-8 md:px-12 lg:px-16">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              {`Located in the heart of Sana'a, we're easy to find and always ready to welcome you.`}
            </p>
          </div>
